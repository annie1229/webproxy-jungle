...
int main(int argc, char **argv) {
    ...
    // SIGPIPE : 끊어진 파이프나 FIFO, 소켓에 데이터 쓰기를 수행한 경우 발생
...
void *thread(void *vargsp) {
    ...
    Pthread_detach(pthread_self()); // 스레드가 종료되면 자원도 반납, pthread_self()는 현재 스레드의 식별자를 리턴한다.
    doit(connfd);
    Close(connfd);
}
...
void doit(int connfd) {
    ...
    Rio_readinitb(&rio, connfd); // rio랑 connfd 연결
    Rio_readlineb(&rio, buf, MAXLINE); // rio로 들어오는 입력 buf에 담기
...
int cache_find(char *url) {
    int i;
    for (i=0; i<CACHE_OBJS_COUNT; i++) {
        readerPre(i); // 캐시 i번 인덱스(공유 자원)에 다른 접근 막기
        if ((cache.cacheobjs[i].isEmpty == 0) && (strcmp(url, cache.cacheobjs[i].cache_url) == 0)) // 접속하려는 url이 캐시에 있으면 break
            break;
        readerAfter(i); // 캐시 i번 인덱스(공유 자원)에 다른 접근 막던거 해제
    }
